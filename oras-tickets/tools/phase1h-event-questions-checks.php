...
oras_phase1h_assert( 2 === substr_count( $board_reports, 'new Observer_Pass_Report_Service()' ), 'Observer dashboard and print handler obtain independent normalized report snapshots' );
...
$agenda_ui_file     = dirname( __DIR__ ) . '/assets/js/agenda-ui.js';
...
oras_phase1h_assert( false !== strpos( $agenda_ui_code, 'tab.tabIndex = tabSelected ? 0 : -1;' ), 'Agenda day activation maintains one keyboard tab stop' );
oras_phase1h_assert( false !== strpos( $agenda_ui_code, "case 'ArrowRight':" ) && false !== strpos( $agenda_ui_code, "case 'ArrowLeft':" ), 'Agenda day tabs support wrapping arrow-key navigation' );
oras_phase1h_assert( false !== strpos( $agenda_ui_code, "case 'Home':" ) && false !== strpos( $agenda_ui_code, "case 'End':" ), 'Agenda day tabs support first and last keyboard navigation' );
oras_phase1h_assert( false !== strpos( $agenda_ui_code, 'function applyFilters(agenda)' ), 'Agenda UI applies type and location filters' );
oras_phase1h_assert( false !== strpos( $agenda_ui_code, 'function getFilterValue(agenda, filterName)' ) && false !== strpos( $agenda_ui_code, "getFilterValue( agenda, 'type' )" ) && false !== strpos( $agenda_ui_code, "getFilterValue( agenda, 'location' )" ), 'Agenda filters read both session type and location controls' );
oras_phase1h_assert( false !== strpos( $agenda_ui_code, "card.dataset.agendaType === activeType" ) && false !== strpos( $agenda_ui_code, "card.dataset.agendaLocation === activeLocation" ), 'Agenda cards must match both active filters' );
