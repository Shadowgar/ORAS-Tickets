...
function oras_phase5_run_checks(): void {
	...
	try {
		...
		$response = oras_phase5_call_json_handler(
			array( Event_RSVP::class, 'handle_post' ),
			array(
				'event_id'  => (string) $concurrency_event_id,
				'intent'    => 'yes',
				'oras_ajax' => '1',
			),
			$user_1,
			'oras_rsvp_' . $concurrency_event_id,
			'oras_rsvp_nonce'
		);
		oras_phase5_assert_same( $response['data']['status'] ?? '', 'yes', 'Concurrency fixture user 1 RSVP yes succeeds' );
		...
		oras_phase5_assert_same( $response['data']['status'] ?? '', 'waitlist', 'Concurrency fixture user 2 falls back to waitlist at RSVP capacity' );

		$interleaved_order = wc_create_order(
			array(
				'customer_id' => $user_4,
			)
		);
		oras_phase5_assert( is_object( $interleaved_order ), 'Interleaved concurrency fixture Woo order created' );
		$interleaved_order->add_product( wc_get_product( $concurrency_product_id ), 2 );
		$interleaved_order->calculate_totals();
		$interleaved_order_id = (int) $interleaved_order->get_id();
		oras_phase5_assert( $interleaved_order_id > 0, 'Interleaved concurrency fixture Woo order persisted' );
		$created_posts[] = $interleaved_order_id;

		$capacity_consumption->handle_paid_order( $interleaved_order_id );
		$capacity_after_interleaved_paid = oras_phase5_get_ticket_capacity( $concurrency_event_id, 0 );
		oras_phase5_assert_same( $capacity_after_interleaved_paid, 3, 'Interleaved paid transition still applies ticket capacity invariant' );
		oras_phase5_assert_same( Waitlist_Store::get_current_waitlist_status( $concurrency_event_id, $user_2 ), 'waiting', 'Interleaved paid transition does not mutate waitlist lifecycle' );
		oras_phase5_assert_same( Event_RSVP::yes_count( $concurrency_event_id ), 1, 'Interleaved paid transition preserves RSVP yes count' );

		$capacity_consumption->handle_restore_order( $interleaved_order_id );
		$capacity_after_interleaved_restore = oras_phase5_get_ticket_capacity( $concurrency_event_id, 0 );
		oras_phase5_assert_same( $capacity_after_interleaved_restore, 5, 'Interleaved restore transition returns ticket capacity invariant' );
		oras_phase5_assert_same( Waitlist_Store::get_current_waitlist_status( $concurrency_event_id, $user_2 ), 'waiting', 'Interleaved restore transition preserves waitlist lifecycle' );
		...
		oras_phase5_assert_same( $response['data']['status'] ?? '', 'none', 'Interleaved revoke frees RSVP slot for promotion path' );

		$interleaved_promote_response = oras_phase5_call_json_handler(
			array( $bootstrap, 'handle_waitlist_bulk_promote' ),
			array(
				'event_id' => (string) $concurrency_event_id,
				'count'    => '1',
		...
		oras_phase5_assert_same( ! empty( $interleaved_promote_response['success'] ), true, 'Interleaved bulk promote succeeds after RSVP slot frees' );
		oras_phase5_assert_same( (int) ( $interleaved_promote_response['data']['promoted_count'] ?? 0 ), 1, 'Interleaved bulk promote advances exactly one waitlist user' );
		oras_phase5_assert_same( get_user_meta( $user_2, '_oras_rsvp_event_' . $concurrency_event_id, true ), 'yes', 'Interleaved bulk promote updates RSVP status to yes' );
		oras_phase5_assert_same( Waitlist_Store::get_current_waitlist_status( $concurrency_event_id, $user_2 ), 'promoted', 'Interleaved bulk promote updates waitlist lifecycle to promoted' );
