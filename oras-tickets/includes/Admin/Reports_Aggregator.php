...
final class Reports_Aggregator { // NOSONAR legacy WP class naming
     ...
     */
    public function get_aggregates( int $event_id, array $statuses, array $date_range = array() ): array {
        ...
        $refund_statuses = array_unique( array_merge( $statuses, array( 'cancelled', 'refunded' ) ) );
...
return;
                ...
                $order_oras_qty   = 0;
                $order_by_ticket  = array();
                $order_by_phase   = array();
                ...
                foreach ( $order_items as $item ) {
                    ...
                    if ( (int) $context['event_id'] === (int) $event_id ) {
                        ...
                        if ( ! isset( $order_by_phase[ $ti ] ) ) {
                            $order_by_phase[ $ti ] = array();
                        }
                        if ( ! isset( $order_by_phase[ $ti ][ $phase_key ] ) ) {
                            $order_by_phase[ $ti ][ $phase_key ] = array(
                                'label'           => $phase_label,
                                'qty'             => 0,
                                'gross'           => 0.0,
                                'refunded_qty'    => 0,
                ...
                foreach ( $order->get_refunds() as $refund ) {
                    ...
                    foreach ( $refund_items as $ref_item ) {
                        ...
                        $ref_total = $ref_item instanceof \WC_Order_Item_Product ? abs( (float) $ref_item->get_total() ) : 0.0;
                        ...
                        if ( ! isset( $phase_breakdown[ $ti ] ) ) {
                            $phase_breakdown[ $ti ] = array();
                        }
                        if ( ! isset( $phase_breakdown[ $ti ][ $phase_key ] ) ) {
                            $phase_breakdown[ $ti ][ $phase_key ] = array(
                                'label'           => $phase_label,
                                'qty'             => 0,
                                'gross'           => 0.0,
                                'refunded_qty'    => 0,
                    ...
                    $other_abs_items = 0.0;
                    $other_types     = array( 'shipping', 'fee', 'tax' );
                    foreach ( $other_types as $type ) {
                        $other_items = method_exists( $refund, 'get_items' ) ? $refund->get_items( $type ) : array();
                ...
                if ( $status_allowed && in_array( $order_status, array( 'processing', 'completed' ), true ) && ! $fully_refunded ) {
        ...
        foreach ( $by_ticket as $ticket_key => $data ) {
            ...
            $by_ticket[ $ticket_key ]['presale_qty'] = (int) $by_ticket[ $ticket_key ]['presale_qty'];
            $by_ticket[ $ticket_key ]['after_qty']   = (int) $by_ticket[ $ticket_key ]['after_qty'];
        }

        foreach ( $phase_breakdown as $ticket_key => $phases ) {
            foreach ( $phases as $phase_key => $phase_row ) {
                $qty        = (int) $phase_row['qty'];
                $ref_qty    = (int) $phase_row['refunded_qty'];
                $gross      = (float) $phase_row['gross'];
                $ref_amount = (float) $phase_row['refunded_amount'];
    ...
    private function normalize_statuses( array $statuses ): array {
        $allowed = array( 'processing', 'completed', 'refunded', 'cancelled' );
        $clean   = array();
