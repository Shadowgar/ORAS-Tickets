...
final class Event_Agenda_Render
{
  private const META_KEY = '_oras_agenda_v1';

  public static function register(): void
  {
    add_filter('the_content', [self::class, 'append_to_content'], 20);
  }
  ...
  public static function append_to_content(string $content): string
  {
    ...
    $days = isset($envelope['days']) && is_array($envelope['days']) ? $envelope['days'] : [];
    if (empty($days)) {
      return $content;
    }
    ...
    if ($highlight_current) {
      ...
      wp_localize_script('oras-agenda-now', 'ORAS_AGENDA_NOW', [
        'tz' => $timezone,
    ...
    foreach ($days as $day_index => $day) {
      ...
      foreach ($slots as $slot_index => $slot) {
        ...
        $speakers = isset($slot['speakers']) && is_array($slot['speakers']) ? $slot['speakers'] : [];
