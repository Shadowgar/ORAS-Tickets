...
final class Event_Agenda_Metabox
{
  ...
  public static function register(): void
  {
    ...
    add_action('save_post_tribe_events', [self::class, 'save'], 10, 1);
  }
  ...
  public static function render(\WP_Post $post): void
  {
    ...
    $settings = isset($envelope['settings']) && is_array($envelope['settings']) ? $envelope['settings'] : [];
    $days = isset($envelope['days']) && is_array($envelope['days']) ? array_values($envelope['days']) : [];
    ...
    <div id="oras-agenda-metabox">
      ...
      <div id="oras-agenda-days">
        <?php foreach ($days as $day_index => $day) : ?>
          ...
          $slots = isset($day['slots']) && is_array($day['slots']) ? $day['slots'] : [];
          ?>
            ...
            <table class="widefat striped oras-agenda-slots-table">
              ...
              <tbody class="oras-agenda-slots-body">
                <?php foreach ($slots as $slot_index => $slot) : ?>
                  ...
                  $slot_speakers = isset($slot['speakers']) && is_array($slot['speakers']) ? $slot['speakers'] : [];
                  ?>
...
<?php
  ...
  public static function save(int $post_id): void
  {
    ...
    $input = isset($_POST['oras_agenda']) && is_array($_POST['oras_agenda']) ? $_POST['oras_agenda'] : [];
    $input_settings = isset($input['settings']) && is_array($input['settings']) ? $input['settings'] : [];
    ...
    $raw_days = isset($input['days']) && is_array($input['days']) ? $input['days'] : [];
    $days = [];
    ...
    foreach ($raw_days as $raw_day) {
      ...
      $raw_slots = isset($raw_day['slots']) && is_array($raw_day['slots']) ? $raw_day['slots'] : [];
      $slots = [];
      ...
      foreach ($raw_slots as $raw_slot) {
        ...
        $raw_speakers = isset($raw_slot['speakers']) && is_array($raw_slot['speakers']) ? $raw_slot['speakers'] : [];
        $speakers = [];
  ...
  {
    return ['talk', 'workshop', 'break', 'observation', 'social', 'other'];
  }
