...
`scripts/run-registration-desk-integration-checks.sh` resolves wp-env configuration and Compose ownership from `/home/rocco/projects/oras-wp-env`, then applies a temporary Compose overlay only to `tests-wordpress` and `tests-cli`. It refuses unknown repository/toolchain identities, pre-existing test overrides it cannot reproduce, wrong project/database/volume/mount/URL identities, mismatched disposable markers, dirty feature code, or missing transport guards. All WordPress checks before marker verification skip ordinary plugins and themes and prove that ORAS Tickets did not bootstrap; must-use outbound guards remain active. The one-time `--initialize-disposable-marker` mode may establish a missing marker only after the designated project, isolated test database volume, loopback URL, exact feature mounts, guards, and mounted-code digest have all been verified.

Before mutation, the runner snapshots ordinary development container identity, state, mounts, environment, and database volume. It also captures full test-service configuration/state, active plugins, ORAS settings, and WooCommerce storage options. Cleanup reads back the restored plugin/settings values, recreates only the test application and CLI containers from the designated base Compose file, compares the restored test configuration with its original snapshot, returns all three test services to their original state, and verifies the ordinary development snapshot is unchanged. Missing dependencies fail closed rather than installing persistent plugin files.
...
Qualification on 2026-09-17 used wp-env 10.39.0 from `/home/rocco/projects/oras-wp-env`, configuration `/home/rocco/projects/oras-wp-env/.wp-env.json`, generated install `/home/rocco/wp-env/a3544f17121d4efebaa9174fe1458a62`, Compose project `a3544f17121d4efebaa9174fe1458a62`, URL `http://localhost:8889`, database `tests-wordpress@tests-mysql`, and volume `a3544f17121d4efebaa9174fe1458a62_mysql-test`. The feature mount was the isolated worktree's `oras-tickets/` at tested code commit `4efd6c098dc62a41a6742ef7ac299e71d94d0439`, with mounted tree digest `c19c7e10e6ca8829c971e0c2a5cfe4f84879efed5d32326e097fba3ff82ed5a9`.

The designated database initially lacked the Registration Desk marker. `--initialize-disposable-marker --verify-environment-only` created `oras-registration-desk-m1a-a3544f17121d4efe` after verifying the distinct development/test volumes, loopback URL, exact project/container labels, both outbound guards, feature mounts, and code digest. A later review found those WordPress identity commands could still bootstrap active plugins before marker rejection. The final runner uses `--skip-plugins --skip-themes` for every pre-marker WordPress command and verifies `oras_plugin_loaded=false`. A deliberate mismatched-marker probe then failed before any ORAS plugin bootstrap warning. The external probe wrapper initially retained stale app/CLI container IDs after runner recreation; freshly resolved IDs were used immediately to restore and verify the exact marker and stopped service state.
...
The final legacy run explicitly reported legacy storage active and compatibility synchronization disabled. The final HPOS run explicitly reported authoritative HPOS active and compatibility synchronization disabled. Both passed the complete WordPress/WooCommerce/TEC suite, authenticated admin-AJAX and WC-AJAX probes, configuration and attendance races, nonfinancial/no-account side-effect snapshots, checkout controls, core regressions, and bootstrap regressions.

Pre-acceptance attempts were recorded rather than concealed: the designated database initially lacked a marker; the first volume-name assertion used the wrong wp-env suffix; an active older ORAS Tickets copy loaded before the feature mount; and the integration harness retained the old worktree URL. A review of the first nominally passing pair then found the pre-marker bootstrap, sync-readback, option-readback, and test-configuration restoration gaps described above. Those results were superseded by the final `4efd6c0` legacy and HPOS reruns after correction. The stale temporary Compose label left on `tests-mysql` by the earliest probe was repaired from the designated base Compose file while retaining its `mysql-test` volume.

The exact style command was `bash scripts/qbo-phpcs-diff-check.sh --baseline a6062bc703394193ef73fbce60742bffd9843679`. Its committed-tree scope was the 19 changed PHP files: plugin paths used `config/phpcs.xml`, while scripts, fixtures, and plugin tools used `config/phpcs-tests.xml`. It reported 0 introduced diagnostics after correcting the 34 reviewed diagnostics. The same run reported 2,481 legacy differential diagnostics and whole-plugin inherited debt of 41,922 errors plus 762 warnings across 91 files; repository-wide style cleanliness is not claimed.

After each acceptance run, the runner restored and read back the test plugin list, ORAS settings snapshot, `woocommerce_custom_orders_table_enabled=no`, missing `woocommerce_custom_orders_table_data_sync_enabled`, base plugin mounts, full test configuration, and the original stopped state of `tests-mysql`, `tests-wordpress`, and `tests-cli`. The ordinary `mysql`, `wordpress`, and `cli` containers remained stopped with identical identities, mounts, environment, and development database volume. The separate worktree-derived project `e3ae9621ee9cf4297eb317ed90aded38` had all six containers running during both acceptance runs and was not used, stopped, recreated, or deleted by the runner. A later final inventory found all six exited with Docker status 255 at the same timestamp; they and their volumes remain present and were not restarted or removed.
