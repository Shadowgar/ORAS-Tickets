...
CHECK_FILE="$ROOT_DIR/scripts/reports-integration-checks.php"

if [[ ! -f "$CHECK_FILE" ]]; then
	echo "Reports check script is missing: $CHECK_FILE" >&2
	exit 1
fi
...
(
	...
	if ! bash -lc "$WP_ENV_CMD run cli wp eval 'exit( class_exists( \"Tribe__Events__Main\" ) ? 0 : 1 );'" >/dev/null 2>&1; then
		echo "Installing The Events Calendar for reports integration checks..."
	...
	if ! bash -lc "$WP_ENV_CMD run cli wp plugin is-installed woocommerce >/dev/null 2>&1"; then
		echo "Installing WooCommerce for reports integration checks..."
		...
		echo "Activating WooCommerce for reports integration checks..."
	...
	bash -lc "$WP_ENV_CMD run cli sh -lc 'echo \"$CHECK_FILE_B64\" | base64 -d > /tmp/oras-reports-integration-checks.php && wp eval-file /tmp/oras-reports-integration-checks.php'"
)
