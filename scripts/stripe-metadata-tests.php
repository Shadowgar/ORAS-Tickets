...
oras_qbo_fixture_run_with_cleanup(
	static function (): void {
		...
		if ( is_wp_error( $event_id ) ) {
			// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Test failure includes a WP_Error diagnostic.
			throw new RuntimeException( 'Failed to create Stripe metadata fixture event: ' . $event_id->get_error_message() );
		}
		$event_id = oras_qbo_fixture_track_post( $event_id );

		$mixed_order = oras_qbo_fixture_track_order( wc_create_order() );
		...
		$observer_only_order = oras_qbo_fixture_track_order( wc_create_order() );
		...
		$unmapped_order = oras_qbo_fixture_track_order( wc_create_order() );
		...
		$long_order      = oras_qbo_fixture_track_order( wc_create_order() );
